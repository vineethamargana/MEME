...
export default async function updateMemebyId(req: Request) {
    try {
        ...
        const user_id = req.headers.get("user_id"); // User ID from headers
        const is_admin = req.headers.get("is_admin") === "true"; // Admin flag
        ...
        const result = await updateMemebyIdRepo(meme, meme_id, user_id, is_admin);
